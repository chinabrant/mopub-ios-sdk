...
@interface MPRewardedAdManagerTests : XCTestCase

@property (nonatomic, strong) MPRewardedAdManager *adManager;
@property (nonatomic, strong) MPRewardedAdManagerDelegateMock *delegateMock;
@property (nonatomic, strong) MPProxy *mockProxy;

@end

@implementation MPRewardedAdManagerTests
...
- (void)setUp {
    ...
    self.mockProxy = [[MPProxy alloc] initWithTarget:[MPRewardedAdManagerDelegateMock new]];
    self.delegateMock = (MPRewardedAdManagerDelegateMock *)self.mockProxy;
    self.adManager = [[MPRewardedAdManager alloc] initWithAdUnitID:kTestAdUnitId delegate:self.delegateMock];
...
- (void)testRewardedSingleCurrencyPresentationSuccess {
    ...
    [self.mockProxy registerSelector:@selector(rewardedAdShouldRewardUserForAdManager:reward:)
    ...
    [self.adManager presentRewardedAdFromViewController:nil withReward:nil customData:nil];
...
- (void)testRewardedMultiCurrencyPresentationSuccess {
    ...
    [self.adManager loadWithConfiguration:config];
    [self.adManager presentRewardedAdFromViewController:nil
...
- (void)testRewardedMultiCurrencyPresentationAutoSelectionFailure {
    ...
    [self.mockProxy registerSelector:@selector(rewardedAdDidFailToShowForAdManager:error:)
                       forPostAction:^(NSInvocation *invocation) {
        rewardForUser = nil;
        didFail = YES;
        [expectation fulfill];
...
- (void)testRewardedMultiCurrencyPresentationUnknownSelectionFail {
    ...
    [self.adManager loadWithConfiguration:config];
    [self.adManager presentRewardedAdFromViewController:nil withReward:badReward customData:nil];
...
- (void)testEmptyConfigurationArray {
    ...
    [self.mockProxy registerSelector:@selector(rewardedAdDidFailToLoadForAdManager:error:)
                       forPostAction:^(NSInvocation *invocation) {
        [expectation fulfill];
    }];
...
- (void)testMultipleResponsesFirstSuccess {
    ...
    [self.mockProxy registerSelector:@selector(rewardedAdDidLoadForAdManager:) forPostAction:^(NSInvocation *invocation) {
        [expectation fulfill];
    }];
    [self.mockProxy registerSelector:@selector(rewardedAdDidFailToLoadForAdManager:error:)
                       forPostAction:^(NSInvocation *invocation) {
...
- (void)testMultipleResponsesMiddleSuccess {
    ...
    [self.mockProxy registerSelector:@selector(rewardedAdDidLoadForAdManager:)
                       forPostAction:^(NSInvocation *invocation) {
        [expectation fulfill];
    }];
    [self.mockProxy registerSelector:@selector(rewardedAdDidFailToLoadForAdManager:error:)
...
- (void)testViewabilityPOSTParameter {
    // Rewarded ads should send a viewability query parameter.
    MPMockAdServerCommunicator * mockAdServerCommunicator = nil;
    MPRewardedAdManager * rewardedAd = [[MPRewardedAdManager alloc] initWithAdUnitID:kTestAdUnitId delegate:nil];
    ...
    [rewardedAd loadRewardedAdWithCustomerId:nil targeting:nil];
...
- (void)testLocalExtrasInCustomEvent {
    ...
    [self.adManager loadRewardedAdWithCustomerId:@"CUSTOMER_ID" targeting:targeting];
...
- (void)testImpressionDelegateFiresWithoutILRD {
    ...
    [self.mockProxy registerSelector:@selector(rewardedAdDidLoadForAdManager:)
        ...
        // Simulate impression to @c MPRewardedAds proper
        [[MPRewardedAds sharedInstance] rewardedAdManager:strongSelf.adManager didReceiveImpressionEventWithImpressionData:nil];
    }];

    [self.mockProxy registerSelector:@selector(rewardedAdManager:didReceiveImpressionEventWithImpressionData:)
                       forPostAction:^(NSInvocation *invocation) {
...
- (void)testImpressionDelegateFiresWithILRD {
    ...
    [self.mockProxy registerSelector:@selector(rewardedAdDidLoadForAdManager:)
        ...
        // Simulate impression to @c MPRewardedAds proper
        [[MPRewardedAds sharedInstance] rewardedAdManager:strongSelf.adManager
